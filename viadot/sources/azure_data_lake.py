...
class AzureDataLake(Source):
    ...
    def to_df(
        ...
        if os.path.splitext(os.path.split(path)[1])[1]:
            ...
            files = self.ls(path)
            dfs = []
            if os.path.splitext(os.path.split(files[0])[1])[1]:
                for file in files:           
                    print(file)
                    df = pd.read_json(
    ...
    def walk(self, path: str = None, depth: int = -1):
        path = path or self.path
        folder_names = []
        file_names = []
        def recurse(self, sub_f, depth):
            folder_names.append(sub_f)
            if sub_f.endswith(".csv") or sub_f.endswith(".parquet") or sub_f.endswith(".json"):
                file_names.append(sub_f)
            elif depth != 0:
                for f_name in self.fs.ls(sub_f):
                    recurse(f_name, depth - 1)
        recurse(path, depth)
        return file_names